...
export class SendGridVerifyTenantEmailService implements IVerifyTenantEmailService {
  ...
  public async verify(tenant: Tenant): Promise<Result<null, ExternalServiceError>> {
    ...
    const response = await this.sendGrid.makeRequest<SendGridRequest<SendGridVerifyEmailDto>, void>(
      request
    );

    if (!response.meta.success) {
      const specificError = this.formatVerifyTenantEmailErrorResponse(
        response.meta.error as unknown as VerifyTenantEmailErrorResponse
      );
      return Result.err(
